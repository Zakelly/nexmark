...
public class FlinkRestClient {
	...
	public synchronized boolean isJobHasTimer(String jobId) {
		String url = String.format("http://%s/jobs/%s", jmEndpoint, jobId);
		String response = executeAsString(url);
		return response.contains("Window");
	}
	...
	public Savepoint getSavepoint(String jobId, String triggerId) {
		String url = String.format("http://%s/jobs/%s/savepoints/%s", jmEndpoint, jobId, triggerId);
		String response = executeAsString(url);
		try {
			JsonNode jsonNode = NexmarkUtils.MAPPER.readTree(response);
			Savepoint.Status status = Savepoint.Status.valueOf(jsonNode.get("status").get("id").asText());
			return new Savepoint(
					status,
					status == Savepoint.Status.COMPLETED ? jsonNode.get("operation").get("location").asText() : null
			);
		} catch (Throwable e) {
			throw new RuntimeException("The response is not a valid JSON string:\n" + response, e);
		}
